...
{$HINTS ON}
  ...
  public
    ...
    procedure Subscribe( filter: AnsiString ); overload;
    procedure subscribe( filter: UInt16 ); overload;                // a two-byte word that defines the messageType ID.
...
begin
  ...
  try
    ...
    begin
      frame := TZMQFrame.create;
      frame.asHexString := AnsiString(tsl[i]);
      add( frame );
    end;
  finally
    tsl.Free;
...
begin
  ...
  result := AnsiString(fAcceptFilter[indx]);
end;
...
procedure TZMQSocket.subscribe( filter: UInt16 );    // a two-byte word that defines the messageType ID.
begin
  if filter = 0 then
    setSockOpt( ZMQ_SUBSCRIBE, nil, 0 )
  else
    setSockOpt( ZMQ_SUBSCRIBE, @filter, 2 );
end;
...
begin
  ...
  fMonitorRec.Addr := AnsiString('inproc://monitor.' + IntToHex( Integer( SocketPtr ),8 ));
...
begin
  ...
  begin
    msg := TZMQFrame.create;
    try
      rc := recv( msg, flags );     // This may generate an exception.
      if rc <> -1 then
      begin
        msgs.Add( msg );
        inc( result );
...
begin
  CheckResult( zmq_ctx_shutdown( fContext ) );
end;
...
begin
  ...
  begin
    ...
    fAddr := AnsiString('inproc://poller' + IntToHex( Integer( Self ), 8 ));
...
begin
  ...
  if ctx = nil then
    ...
    fPipe.bind( ansiString(Format( 'inproc://zmqthread-pipe-%p', [@fPipe] ) ));
  end;
end;
...
begin
  ...
  begin // attached thread
    ...
    thrPipe.connect( AnsiString(Format( 'inproc://zmqthread-pipe-%p', [@fPipe] ) ));
  end;
